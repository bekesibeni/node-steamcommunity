...
declare module '../SteamCommunity' {
	interface SteamCommunity {
		...
		getUserComments(
			...
			options: Record<string, unknown> | Callback<[UserComment[], number]>,
			callback?: Callback<[UserComment[], number]>,
		): void;
		...
		getUserAliases(
			userID: SteamID | string,
			callback: Callback<Array<{ newname: string; timechanged: Date }>>,
		): void;
		getUserProfileBackground(userID: SteamID | string, callback: Callback<string | null>): void;
		getUserInventoryContexts(
			userID: SteamID | string | Callback<Record<string, unknown>>,
...
SteamCommunity.prototype.parentalUnlock = function (
	this: SteamCommunity,
	pin: string,
...
SteamCommunity.prototype.getNotifications = function (
	this: SteamCommunity,
	callback: Callback<SteamNotifications>,
): void {
	this.httpRequestGet(
		{
		...
		(err, _response, body) => {
			...
			const b = body as Record<string, Record<string, number>> | null;
...
SteamCommunity.prototype.resetItemNotifications = function (
	this: SteamCommunity,
	callback?: SimpleCallback,
): void {
	this.httpRequestGet(
		{ uri: 'https://steamcommunity.com/my/inventory' },
		(err) => { callback?.(err); },
		'steamcommunity',
	);
};
...
SteamCommunity.prototype.loggedIn = function (
	this: SteamCommunity,
	callback: (err: Error | null, loggedIn?: boolean, familyView?: boolean) => void,
): void {
	this.httpRequest(
		{
...
SteamCommunity.prototype.getTradeURL = function (
	this: SteamCommunity,
	callback: (err: Error | null, url?: string, token?: string) => void,
): void {
	this._myProfile(
		'tradeoffers/privacy',
		null,
...
SteamCommunity.prototype.changeTradeURL = function (
	this: SteamCommunity,
	callback?: (err: Error | null, url?: string, token?: string) => void,
): void {
	this._myProfile(
		...
		(err, _response, body) => {
			...
			try {
				parsed = JSON.parse(typeof body === 'string' ? body : JSON.stringify(body)) as Record<string, unknown>;
			} catch {
				callback?.(new Error('Malformed response'));
				return;
			}
			...
			const url = `https://steamcommunity.com/tradeoffer/new/?partner=${
...
SteamCommunity.prototype.clearPersonaNameHistory = function (
	this: SteamCommunity,
	callback?: SimpleCallback,
): void {
	this._myProfile(
...
SteamCommunity.prototype.getFriendsList = function (
	this: SteamCommunity,
	callback: Callback<Record<string, number>>,
): void {
	this.httpRequestGet(
		{
		...
		(err, _response, body) => {
			...
			if (Number(b?.['success']) !== 1) {
				const eresultErr = Helpers.eresultError(Number(b?.['success'] ?? 0));
				callback(eresultErr ?? new Error('Unknown error'), null!);
				return;
			}

			const friends = (b?.['friendslist'] as Record<string, unknown>)?.['friends'];
			const result: Record<string, number> = {};

			if (Array.isArray(friends)) {
				for (const entry of friends as Array<Record<string, unknown>>) {
...
SteamCommunity.prototype.addFriend = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.acceptFriendRequest = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.removeFriend = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.blockCommunication = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.unblockCommunication = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.postUserComment = function (
	...
	this.httpRequestPost(
		...
		(err, _response, body) => {
			...
			const idAttr = parse(html).querySelector('.commentthread_comment')?.getAttribute('id') ?? '';
		...
		(err, _response, body) => {
			...
			for (const el of commentsRoot.querySelectorAll('.commentthread_comment.responsive_body_text[id]')) {
				...
				const steamID = miniprofile
					? new SteamID('[U:1:' + miniprofile + ']')
					: new SteamID(authorElem?.getAttribute('href')?.match(/\/profiles\/(\d+)/)?.[1] ?? '0');
				...
				comments.push({
					...
					date: Helpers.decodeSteamTime(dateStr),
...
SteamCommunity.prototype.followUser = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.unfollowUser = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback?: SimpleCallback,
): void {
...
SteamCommunity.prototype.getUserAliases = function (
	this: SteamCommunity,
	userID: SteamID | string,
	callback: Callback<Array<{ newname: string; timechanged: Date }>>,
): void {
	...
	this.httpRequestGet(
		...
		(err, _response, body) => {
			...
			const raw = body as Array<Record<string, string>> | null;
			...
			const result = raw.map((entry) => ({
				...
				timechanged: Helpers.decodeSteamTime(String(entry['timechanged'] ?? '')),
			}));
...
SteamCommunity.prototype.getUserProfileBackground = function (
	...
	const sid = toSteamID(userID);
	this.httpRequestGet(
		{ uri: `https://steamcommunity.com/profiles/${sid.getSteamID64()}` },
		(err, _response, body) => {
			if (err) { callback(err, null!); return; }

			const html = typeof body === 'string' ? body : '';
			const bgRoot = parse(html);
...
SteamCommunity.prototype.getUserInventoryContexts = function (
	this: SteamCommunity,
	userID: SteamID | string | Callback<Record<string, unknown>>,
	callback?: Callback<Record<string, unknown>>,
): void {
	...
	if (typeof userID === 'function') {
		callback = userID as Callback<Record<string, unknown>>;
		sid = this.steamID ?? new SteamID();
	} else {
		sid = toSteamID(userID as SteamID | string);
	}

	this.httpRequestGet(
		{ uri: `https://steamcommunity.com/profiles/${sid.getSteamID64()}/inventory/` },
		(err, _response, body) => {
			if (err) { callback?.(err, null!); return; }
...
/** @deprecated Use getUserInventoryContents instead. */
SteamCommunity.prototype.getUserInventory = function (
	this: SteamCommunity,
	userID: SteamID | string,
	appID: number,
	contextID: number,
	...
	const fetchPage = (start?: number): void => {
		...
		this.httpRequestGet(
			...
			(err, _response, body) => {
				...
				const rgCurrency = (b['rgCurrency'] ?? {}) as Record<string, Record<string, unknown>>;
				...
				for (const assetid of Object.keys(rgInventory)) {
					...
					const item = new CEconItem(asset, desc, contextID);
...
SteamCommunity.prototype.getUserInventoryContents = function (
	this: SteamCommunity,
	userID: SteamID | string,
	appID: number,
	contextID: number,
	...
	const fetchPage = (startAssetID?: string): void => {
		const qs: Record<string, unknown> = {
			l: lang,
			count: 1000,
		};
		...
		this.httpRequestGet(
			...
			(err, _response, body) => {
				if (err) { (callback as Function)(err); return; }

				const b = body as Record<string, unknown> | null;
				if (!b || (!b['assets'] && !b['inventory'])) {
					if (!b?.['success'] && b?.['error']) {
						(callback as Function)(new Error(String(b['error'])));
					} else if (startAssetID === undefined && !inventory.length && !currency.length) {
						// Empty inventory is valid
						(callback as Function)(null, inventory, currency, 0);
					} else {
						(callback as Function)(null, inventory, currency, Number(b?.['total_inventory_count'] ?? 0));
					}
					return;
				}
				...
				const rawDescs = (b['descriptions'] ?? []) as Array<Record<string, unknown>>;
				...
				const assetPropsMap: Record<string, RawAssetPropertiesEntry> = {};
				const rawAssetProps = (b['asset_properties'] ?? []) as Array<RawAssetPropertiesEntry>;
				...
				function getDescription(classid: string, instanceid: string): Record<string, unknown> | null {
					return descriptions[`${classid}_${instanceid}`] ?? null;
				}

				function getAssetProperties(assetid: string): RawAssetPropertiesEntry | undefined {
				...
				for (const asset of assets) {
					...
					const item = new CEconItem(asset, desc, contextID, assetProps);
					if (item.is_currency) {
						if (!tradableOnly || item.tradable) {
							currency.push(item);
						}
				...
				if (b['more_items']) {
					...
					(callback as Function)(null, inventory, currency, totalInventoryCount);
				}
			},
			'steamcommunity',
		);
...
SteamCommunity.prototype.sendImageToUser = function (
	...
	const opts = options as { spoiler?: boolean };
	const sid = toSteamID(userID);
	const sessionid = this.getSessionID();

	// Compute file metadata
	const fileSha = Crypto.createHash('sha1').update(imageContentsBuffer).digest('hex');
	const fileSize = imageContentsBuffer.length;

	let dimensions: { width: number; height: number };
	try {
		const sized = imageSize(imageContentsBuffer);
	...
	this.httpRequestPost(
		...
		(err, _response, body) => {
			...
			const startResult = body as Record<string, unknown> | null;
			...
			const requestHeaders = (startResult['request_headers'] ?? []) as Array<Record<string, string>>;
			...
			this.httpRequest(
				...
				(putErr) => {
					if (putErr) { callback?.(putErr, null!); return; }
					...
					this.httpRequestPost(
						...
						(commitErr, _commitResponse, commitBody) => {
							if (commitErr) { callback?.(commitErr, null!); return; }

							const cb = commitBody as Record<string, unknown> | null;
							const resultUrl = (
								(cb?.['result'] as Record<string, unknown> | undefined)?.['details'] as Record<string, unknown> | undefined
							)?.['url'];
